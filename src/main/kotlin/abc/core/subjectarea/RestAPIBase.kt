...
class RestAPIBase (var ip: String = "127.0.0.1", var port: Int = 4567){
    ...
    suspend fun downLoadByName(SessionToken:String,fname:String,root:Boolean) : ResponseBody {
        val response = window
        .fetch("http://"+ip+":"+port+"/api/file/loadByName?fname="+fname+"&root="+root+"",RequestInit("get",Headers().append("SessionToken",SessionToken)))
            .await().text().await()
        val format = Json { ignoreUnknownKeys = true }
        return format.decodeFromString<ResponseBody>(response)
        }
    ...
    suspend fun downLoad2(SessionToken:String) : ResponseBody {
        val response = window
        .fetch("http://"+ip+":"+port+"/api/file/load2",RequestInit("get",Headers().append("SessionToken",SessionToken)))
            .await().text().await()
        val format = Json { ignoreUnknownKeys = true }
        return format.decodeFromString<ResponseBody>(response)
        }
